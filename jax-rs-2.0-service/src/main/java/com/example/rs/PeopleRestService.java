...
public class PeopleRestService {
	...
	public Collection< Person > getPeople( @QueryParam( "page") @DefaultValue( "1" ) final int page ) {
		System.out.println("Processing request on port "+environment.getProperty( AppConfig.SERVER_PORT));
		try {
			Thread.sleep(100);
		} catch (InterruptedException e) {
		}
		System.out.println("Returning from port "+environment.getProperty( AppConfig.SERVER_PORT));
